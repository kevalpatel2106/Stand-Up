...
package com.kevalpatel2106.common.notifications
...
object NotificationChannelType {
    ...
    /**
     * Notifications for generalized activities.
     */
    const val OTHER_NOTIFICATION_CHANNEL = "other_notification_channel"
     ...
     * Notifications for reminder activities.
     */
    const val DAILY_SUMMARY_NOTIFICATION_CHANNEL = "daily_summary_notification_channel"
}
...
@TargetApi(Build.VERSION_CODES.O)
fun NotificationManager.addDailySummaryNotificationChannel(context: Context) {

    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        val chan2 = NotificationChannel(NotificationChannelType.DAILY_SUMMARY_NOTIFICATION_CHANNEL,
                context.getString(R.string.notification_channel_title_reminders), NotificationManager.IMPORTANCE_HIGH)
        chan2.lightColor = Color.CYAN
        chan2.lockscreenVisibility = Notification.VISIBILITY_PUBLIC
...
@TargetApi(Build.VERSION_CODES.O)
fun NotificationManager.addOtherNotificationChannel(context: Context) {

    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        val chan2 = NotificationChannel(NotificationChannelType.OTHER_NOTIFICATION_CHANNEL,
                context.getString(R.string.notification_channel_title_other), NotificationManager.IMPORTANCE_LOW)
