 ...
 * row_detected_activity layout and populates each element with data from a DetectedActivity
 * object.
 */
class DetectedActivitiesAdapter extends ArrayAdapter<DetectedActivity> {
    ...
    public View getView(int position, @Nullable View view, @NonNull ViewGroup parent) {
        ...
        if (view == null) {
            view = LayoutInflater.from(getContext()).inflate(
                    R.layout.row_detected_activity, parent, false);
        }
        ...
        if (detectedActivity != null) {
            activityName.setText(DemoRecognitionActivity.getActivityString(detectedActivity.getType()));
    ...
    void updateActivities(ArrayList<DetectedActivity> detectedActivities) {
        ...
        for (int i = 0; i < DemoRecognitionActivity.MONITORED_ACTIVITIES.length; i++) {
            int confidence = detectedActivitiesMap.containsKey(DemoRecognitionActivity.MONITORED_ACTIVITIES[i]) ?
                    detectedActivitiesMap.get(DemoRecognitionActivity.MONITORED_ACTIVITIES[i]) : 0;

            tempList.add(new DetectedActivity(DemoRecognitionActivity.MONITORED_ACTIVITIES[i],
                    confidence));
        }
