...
class UpdateNotificationTest {
    ...
    fun setUp() {
        ...
        Mockito.`when`(context.getString(R.string.notification_update_title)).thenReturn(TEST_TITLE_STRING)
        Mockito.`when`(context.resources).thenReturn(resources)

        notification = UpdateNotification.buildNotification(context, TEST_MESSAGE_STRING)
    }

    @Test
    @Throws(IOException::class)
    fun testNotificationPriority() {
        Assert.assertEquals(notification.priority, NotificationCompat.PRIORITY_HIGH)
    }
